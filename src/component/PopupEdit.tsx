...
export default function PopupEdit({ setIsPopupEdit }: popupType): JSX.Element {
  const { catchId, data, setData, setIsUpdate } = ThemProvider();
  ...
  const selectedItem = data.find((item) => item.id === catchId);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updatedData = data.map((item) =>
      item.id === catchId ? updatedItem : item
    );
    // Update the state with the new data
    setData(updatedData as dataType[]);
    // Close the popup
